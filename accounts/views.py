...
def register_page(request):
    if request.method == 'POST':
        ...
        profile.is_email_verified = True  # Langsung aktif
        profile.save()
...
def login_page(request):
    ...
    if request.method == 'POST':
        ...
        # Hapus pengecekan verifikasi email
...
def cart(request):
    ...
    if cart_obj:
        ...
        if cart_total_in_paise < 100:
            messages.warning(
                request, 'Total amount in cart is less than the minimum required amount (1.00 IDR). Please add a product to the cart.')
            return redirect('index')
